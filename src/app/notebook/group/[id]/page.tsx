...
import { Plus, Trash2, ArrowLeft, Users, MoreVertical, UserPlus, Trash } from 'lucide-react';
    ...
    return (
         <Dialog>
            <DialogTrigger asChild>
                <Button className="w-full"><Plus className="mr-2"/> Tambah Tugas</Button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
...
export default function GroupNotebookPage() {
  ...
  useEffect(() => {
    if (!id) return;
    // In a real app, you'd fetch this from your backend. Here we simulate it.
  ...
  const toggleTaskCompletion = useCallback((taskId: string) => {
    setGroup(currentGroup => {
        ...
        const updatedTasks = currentGroup.tasks.map(task =>
            task.id === taskId ? { ...task, completed: !task.completed } : task
        );
        return { ...currentGroup, tasks: updatedTasks };
    });
    // In a real app, you would debounce this and send an update to the server.
  }, []);
  ...
  const getAssigneeAvatars = (task: GroupTask) => {
    ...
    if (task.assignedTo.length === 0) {
        return [{ id: 'all', name: 'Semua', avatarUrl: '' }];
    }
    return group.members.filter(member => task.assignedTo.includes(member.id));
  };
                ...
                <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction onClick={() => handleRemoveTask(task.id)}>Hapus</Button>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
                  ...
                  {group.tasks.length === 0 && (
                      <div className="text-center py-8 text-muted-foreground">
                          <p>Belum ada tugas di grup ini.</p>
                      </div>
                  )}
                </div>
